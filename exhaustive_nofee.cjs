...
for (let sl of [1, 1.5, 2, 2.5, 3, 4, 5]) {
  for (let tact of [0, 1, 2, 3, 4, 5]) {
    for (let tdist of [0, 0.5, 1, 2, 3]) {
      let pnl = 0;
      for (const r of data) {
      ...
      if (Math.abs(pnl - (-212.78)) < 1.0) {
         console.log(`FOUND NO FEE OLD: SL=${sl}, TACT=${tact}, TDIST=${tdist} -> PnL=${pnl.toFixed(2)}`);
      }
    }
  }
}
