...
const regex = /<div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-1 shrink-0">([\s\S]*?)<div className="bg-\[#0d1017\] border border-amber-500\/20 p-2 rounded flex flex-col">(\s*)<span className="text-\[10px\] text-zinc-400 uppercase font-semibold">\{t.activePositions\}<\/span>/;

const newRibbon = `<div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 xl:grid-cols-8 gap-1 shrink-0">
            ...
            <div className="bg-[#0d1017] border border-amber-500/20 p-2 rounded flex flex-col">$2<span className="text-[10px] text-zinc-400 uppercase font-semibold">{t.activePositions}</span>`;

if (regex.test(content)) {
  content = content.replace(regex, newRibbon);
  fs.writeFileSync('src/components/BloombergTerminal.tsx', content);
  console.log('Patched ribbon layout using regex');
} else {
  console.log('Ribbon target not found via regex');
}
