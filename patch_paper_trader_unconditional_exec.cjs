...
const replacement = `          // Execute natively in MomentumExecutor unconditionally so positions are ALWAYS opened in the simulator
          const feePaid = sizeUSDT * (this.config.entryFeePct / 100);
          this.deductCapital(sizeUSDT + feePaid);
          this.state.totalFeesPaid = (this.state.totalFeesPaid || 0) + feePaid;
          const newPos: PaperPosition = {
          ...
          // Also notify external callback if configured
...
console.log('MomentumExecutor.ts patched to execute natively first, then call external callback');
