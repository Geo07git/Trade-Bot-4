...
async function generateOOS() {
  console.log("--- STARTING OOS BACKTEST GENERATION ---");
  const { tradingUniverse } = await getBinanceUniverse();
  
  // Use a smaller universe and shorter time to speed up
  const symbols = tradingUniverse.slice(0, 20); 
  ...
  const oosStartTime = new Date('2025-02-01T00:00:00Z').getTime(); // Use 6 months

  console.log(`Running OOS backtest for ${symbols.length} symbols from ${new Date(oosStartTime).toISOString()} to ${new Date(oosEndTime).toISOString()}`);
  ...
  console.log(`--- OOS BACKTEST COMPLETE. WRITTEN TO ${filename} ---`);
}
