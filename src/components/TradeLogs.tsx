...
export function TradeLogs() {
  ...
  const [signalFilter, setSignalFilter] = useState<'executed' | 'all' | 'BUY' | 'SELL' | 'HOLD' | 'reversal'>('all');
  ...
  const filteredSignals = rawSignalList.filter(s => {
    ...
    if (signalFilter === 'executed') return s.finalAction === 'BUY' || s.finalAction === 'SELL';
  ...
  return (
    <div className="flex flex-col h-full bg-black">
      <header className="py-2.5 sm:py-4 border-b border-white/5 flex items-center justify-between px-3 sm:px-8 bg-zinc-900/10 backdrop-blur-md shrink-0 flex-wrap gap-2 sm:gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="font-serif text-base sm:text-xl text-white">Decision & Audit Journal</h1>
            <span className="text-[9px] sm:text-[10px] bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded-full font-mono flex items-center gap-1">
              <HardDrive className="w-3 h-3" />
              VPS 24/7 Engine
            </span>
          </div>
          <p className="text-[9px] uppercase text-zinc-500 tracking-wider mt-0.5 hidden sm:block">Jurnal de decizie AI & audit detaliat al tuturor semnalelor evaluate</p>
        </div>

        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          {/* Capacity Selector */}
          <div className="flex items-center gap-1.5 bg-zinc-900 border border-white/10 rounded-lg px-2 sm:px-3 py-1">
            <Database className="w-3 h-3 text-emerald-400 shrink-0" />
            <span className="text-[10px] sm:text-xs text-zinc-400 font-medium whitespace-nowrap">Capacitate:</span>
            <select
              value={maxLogs || 1000}
              onChange={(e) => setMaxLogs(Number(e.target.value))}
              className="bg-black text-[10px] sm:text-xs text-emerald-400 font-mono font-semibold focus:outline-none cursor-pointer border border-emerald-500/30 rounded px-1.5 py-0.5"
            >
          ...
          <button 
            ...
            className="text-[9px] sm:text-[10px] uppercase tracking-widest text-rose-400 border border-rose-500/20 px-2.5 py-1 rounded-lg hover:bg-rose-500/10 transition-colors flex items-center gap-1 font-medium"
          >
          ...
          <button 
            ...
            className="text-[9px] sm:text-[10px] uppercase tracking-widest text-emerald-400 border border-emerald-500/20 px-2.5 py-1 rounded-lg hover:bg-emerald-500/10 transition-colors flex items-center gap-1 font-medium"
          >
          ...
          <button 
            ...
            className="text-[9px] sm:text-[10px] uppercase tracking-widest text-zinc-300 border border-white/10 px-2.5 py-1 rounded-lg hover:bg-white/5 transition-colors flex items-center gap-1"
          >
            JSON
          </button>
        </div>
      ...
      <div className="px-3 sm:px-8 pt-3 pb-2 border-b border-white/5 flex items-center justify-between gap-3 flex-wrap bg-zinc-950">
        ...
        {activeTab === 'signals' && (
          <div className="flex items-center gap-1.5 text-[11px] flex-wrap">
            <div className="bg-zinc-900 border border-white/5 rounded-lg px-2 py-0.5 flex items-center gap-1">
              <span className="text-zinc-500 text-[10px]">Cumpărări:</span>
              <span className="font-mono text-emerald-400 font-bold">{buySignalsCount}</span>
            </div>
            <div className="bg-zinc-900 border border-white/5 rounded-lg px-2 py-0.5 flex items-center gap-1">
              <span className="text-zinc-500 text-[10px]">Vânzări:</span>
              <span className="font-mono text-rose-400 font-bold">{sellSignalsCount}</span>
            </div>
            <div className="bg-zinc-900 border border-white/5 rounded-lg px-2 py-0.5 flex items-center gap-1">
              <span className="text-zinc-500 text-[10px]">Respinse:</span>
      ...
      <div className="p-3 sm:p-6 overflow-y-auto flex-1 space-y-3 sm:space-y-4">
        {activeTab === 'signals' ? (
          <>
            ...
            <div className="flex items-center justify-between gap-2.5 flex-wrap bg-zinc-900/60 border border-white/5 rounded-xl p-2.5">
              <div className="flex items-center gap-2 flex-1 min-w-[200px] bg-black border border-white/10 rounded-lg px-2.5 py-1.5">
                <Search className="w-3.5 h-3.5 text-zinc-500" />
                <input 
                  type="text"
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  placeholder="Caută semnal..."
              ...
              <div className="flex items-center gap-1 text-xs flex-wrap">
                <button
                  ...
                  className={`px-2 py-1 rounded-md text-[10px] font-semibold transition-colors flex items-center gap-1 ${
                    signalFilter === 'executed' ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 shadow-sm' : 'text-zinc-400 hover:text-white'
                  }`}
                >
                  ⚡ Tranzacții ({buySignalsCount + sellSignalsCount})
                </button>
                <button
                ...
                >
                  HOLD ({rawSignalList.length - buySignalsCount - sellSignalsCount})
                </button>
              </div>
            </div>

            {/* Mobile Cards for Signals (shown on mobile md:hidden) */}
            <div className="block md:hidden space-y-2">
              {filteredSignals.length === 0 ? (
                <div className="py-8 text-center text-zinc-500 text-xs font-sans bg-zinc-900/50 rounded-xl border border-white/5 p-4">
                  Nicio evaluare de semnal găsită.
                </div>
              ) : (
                filteredSignals.map((sig) => {
                  ...
                  return (
                    <div key={sig.id} className="bg-zinc-900/90 border border-white/10 rounded-xl p-3 space-y-2 text-xs font-mono shadow-md">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1.5">
                          <span className="font-bold text-sm text-white">{sig.symbol}</span>
                          {sig.isReversal && (
                            <span className="bg-cyan-500/20 text-cyan-400 border border-cyan-500/30 text-[9px] px-1 py-0.2 rounded font-mono flex items-center gap-0.5">
                              <Zap className="w-2.5 h-2.5" /> REV
                            </span>
                          )}
                          <span className="text-zinc-400 text-[10px]">{sig.time}</span>
                        </div>
                      ...
                      <div className="text-[10px] text-zinc-300 bg-black/40 rounded p-1.5 border border-white/5 flex items-center justify-between gap-2">
                        <span className="truncate">{sig.vetoReason}</span>
                        <button
                          onClick={() => setSelectedSignal(sig)}
                          className="text-[10px] text-emerald-400 hover:text-emerald-300 bg-emerald-500/10 px-2 py-0.5 rounded transition-colors shrink-0 font-medium"
                        >
                          Info
                        </button>
                      </div>
            ...
            {/* Desktop Signal Audit Table (hidden on mobile md:hidden) */}
            <div className="hidden md:block bg-[#18181b] rounded-xl border border-white/10 overflow-hidden shadow-2xl">
