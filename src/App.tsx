...
import { BacktestMomentum } from './components/BacktestMomentum';
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen w-full bg-black text-zinc-100 overflow-hidden font-sans">
      ...
      <header className="md:hidden h-11 bg-zinc-950 border-b border-white/10 flex items-center justify-between px-2.5 shrink-0 z-30">
        <div className="flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-1.5">
            ...
            />
            <span className="font-mono text-sm text-white font-bold tracking-tight">TradeBot</span>
          </div>
        </div>
        
        <div className="flex items-center gap-1.5">
          <button
            onClick={() => setCurrentView(activeView === 'bloomberg' || activeView === 'superDashboard' ? 'dashboard' : 'bloomberg')}
            className={cn(
              "px-2 py-0.5 rounded-full text-[10px] font-medium border transition-all flex items-center gap-1 cursor-pointer",
              activeView === 'bloomberg' || activeView === 'superDashboard'
                ? "bg-amber-500/20 text-amber-300 border-amber-500/40 font-semibold"
                : "bg-zinc-900 text-zinc-300 border-white/10 hover:text-white"
            )}
            title="Toggle Quick Matrix"
          >
            <span>{t.quickMatrix}</span>
          </button>
          ...
          <button
            ...
            className="px-2 py-0.5 rounded-full text-[10px] font-medium border border-amber-500/40 bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 transition-all flex items-center gap-1 cursor-pointer font-mono"
            title="Hard Refresh: Reset store & reload"
          >
            <RotateCcw className="w-3 h-3 text-amber-400" />
            <span>{t.cmdReset}</span>
          </button>

          <button
            onClick={() => setAutoTradingActive(!autoTradingActive)}
            className={cn(
              "flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold border transition-all cursor-pointer font-mono",
              autoTradingActive
                ? "bg-emerald-500/10 border-emerald-500/40 text-emerald-400"
                : "bg-rose-500/10 border-rose-500/40 text-rose-400"
            )}
            title="Toggle Autonomous Trading"
          >
            <span className={cn("w-1.5 h-1.5 rounded-full", autoTradingActive ? "bg-emerald-400 animate-pulse" : "bg-rose-500")}></span>
            <span>{autoTradingActive ? t.autoTradingOn : t.autoTradingOff}</span>
          </button>
        </div>
      </header>
      ...
      <main className="flex-1 h-full overflow-hidden relative flex flex-col">
        ...
        {circuitBreakerTriggered && (
          <div className="bg-gradient-to-r from-rose-950/90 via-red-900/90 to-rose-950/90 border-b border-rose-500/40 px-4 py-3 text-white flex flex-col md:flex-row items-center justify-between gap-3 shadow-lg z-20 shrink-0 font-mono">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="font-semibold text-sm text-rose-200">{t.emergencyKill}</span>
                  <span className="text-[10px] uppercase font-mono px-2 py-0.5 rounded bg-rose-500/30 text-rose-300 border border-rose-400/30">{t.tradingHalted}</span>
                </div>
                <p className="text-xs text-rose-300/90 mt-0.5">
                  {circuitBreakerReason || "Safety limits reached (+10% profit / -5% drawdown). Auto-execution stopped."}
                </p>
              </div>
            </div>
        ...
        <div className="flex-1 h-full overflow-hidden relative flex flex-col">
          {(activeView === 'bloomberg' || activeView === 'superDashboard') && <BloombergTerminal />}
          {activeView === 'dashboard' && <Dashboard />}
          {(activeView === 'strategy' || activeView === 'scalping') && <ScalpingBot />}
          {activeView === 'audit' && <AuditTrailView />}
          {activeView === 'momentumPaper' && <MomentumPaperView />}
          {activeView === 'backtest' && <BacktestMomentum />}
